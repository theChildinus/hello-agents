...
针对本节设定的目标——回答关于“华为最新手机”的问题，我们需要为智能体提供一个网页搜索工具。但是第一章的尝试可以看出一个简单的搜索引擎库（如DDGS）在实战中可能无法提供足够高质量的信息。为了构建一个真正可靠的智能体，我们需要一个更专业的搜索工具。为此，我们选用 <strong>SerpApi</strong>，它通过API提供结构化的Google搜索结果，能直接返回“答案摘要框”或精确的知识图谱信息，
